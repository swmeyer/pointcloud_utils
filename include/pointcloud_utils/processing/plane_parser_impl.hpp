 ...
 * File: plane_parser_impl.hpp
 */

#ifndef PLANE_PARSER_IMPL_HPP
#define PLANE_PARSER_IMPL_HPP
...
{
	template <class T> PlaneParser<T>::PlaneParser(const plane_parser_utils::Settings& settings)
	{
		this->settings = settings;
	}

	template <class T> PlaneParser<T>::~PlaneParser()
	{

	}

	template <class T> void PlaneParser<T>::updateSettings(plane_parser_utils::Settings& settings)
	{
		this->settings = settings;
	}
	 ...
	 * @return 		void
	 */
	template <class T> void PlaneParser<T>::parsePlane
	(
	...
	)
	{
		...
		if (cloud_in->width < (uint) settings.min_points_to_fit)
		{
		...
		// std::cout << "Processing cloud with " << cloud.size() << " points\n";

		//Vector to save plane points in:
		std::vector<T> filtered_cloud_vector;
	...
	)
	{
		...
		plane_parser_utils::LeastSquaresMatricies least_squares_matricies;
		if (filtered_cloud.size() < (uint) settings.min_points_to_fit)
		{
		...
		getPlaneStates(plane_parameters, plane_states);
	 ...
	 * 				       false otherwise
	 */
	template <class T> bool PlaneParser<T>::pointIsOnPlane(const T& pt, const plane_parser_utils::PlaneParameters& plane_parameters, const float& tolerance)
	{
	 ...
	 */
	template <class T> void PlaneParser<T>::filterCloud
	(
		std::vector<T>& cloud, 
	...
	)
	{
		...
		if (settings.invert_cloud_step_through)
		{
			for (int i = cloud.size() - 1; i >= 0; i -= (settings.point_skip_num + 1))
			{
				// std::cout << "Point: " << i << "\n";
				T pt = cloud[i];
				...
				if (plane_points.size() >= (uint) settings.max_point_count)
				{
					break;
				}
			}
		...
		{
			for (uint i = 0; i < cloud.size(); i += settings.point_skip_num + 1)
			{
				T pt = cloud[i];
				//std::cout << "Point: " << pt.x << ", " << pt.y << ", " << pt.z << ", " << pt.intensity << "\n";
				if (pt.x <= search_window.x_max && pt.x >= search_window.x_min &&
	 ...
	 */
	template <class T> void PlaneParser<T>::fitPlane(const std::vector<T>& cloud, plane_parser_utils::LeastSquaresMatricies& matricies)
	{
		//std::cout << "Fitting plane to " << cloud.size() << " points\n";
		if (cloud.size() < (uint) settings.min_points_to_fit)
		{
		...
		for (T pt : cloud)
		{
			if (i != 0)
			{
				if ( pointcloud_utils::inTolerance(pt.z, prev_height, this->settings.consecutive_height_tolerance) )
				{
					prev_height = pt.z;
				} else
				{
		...
		//Solve least squares:
		if (settings.plane_fit_type == pointcloud_utils::plane_parser_utils::PlaneFitType::SIMPLE)
		{
		...
		else if (settings.plane_fit_type == pointcloud_utils::plane_parser_utils::PlaneFitType::SVD)
		{
	 ...
	 */
	template <class T> int PlaneParser<T>::removeOutliers(std::vector<T>& plane_points, const Eigen::Vector3f& plane_coefficients)
	{
		std::vector<T> saved_points;
		saved_points = plane_points; //TODO: make sure this isn't a linked reference
		plane_points.clear();

		int outlier_count = 0;
		for (T pt : saved_points)
		{
...
//
			...
			plane_parser_utils::PlaneParameters params;
	 ...
	 */
	template <class T> void PlaneParser<T>::getPlaneStates
	(
		const plane_parser_utils::PlaneParameters& plane_parameters,
		plane_parser_utils::States& plane_states
	)
	{
		...
		} else
		{
			...
			T origin_pt; //NOTE: this is designed to work best for a nearly horizontal plane

			// // if goodness of fit, use the variance for all covariance values
			// if (settings.covariance_type == plane_parser_utils::CovarianceType::GOODNESS_OF_FIT_ERROR ||
			// 	settings.covariance_type == plane_parser_utils::CovarianceType::GOODNESS_OF_FIT_DISTANCE)
			// {
	 ...
	 */
	template <class T> void PlaneParser<T>::getPlaneOrientation(const plane_parser_utils::PlaneParameters plane_parameters, Eigen::Matrix3f& orientation_covariance, double& roll, double& pitch, double& yaw, Eigen::Quaternion<float>& quat)
	{
		...
		//if (settings.angle_covariance_type == pointcloud_utils::plane_parser_utils::AngleCovarianceType::LINEAR_APPROXIMATION)
		//{
		...
		//} else if (settings.angle_covariance_type == pointcloud_utils::plane_parser_utils::AngleCovarianceType::HIGHER_ORDER_UNSCENTED_TRANSFORM)
		//{	
...
////
		...
		{
			case (plane_parser_utils::AngleSolutionType::ATTITUDE_ANGLES):
			{
			...
			case (plane_parser_utils::AngleSolutionType::EULER_ANGLES):
			{
		...
		// 
			...
			case (plane_parser_utils::AngleSolutionType::SIMPLE_ANGLES):
			{
			...
			case (plane_parser_utils::AngleSolutionType::QUATERNIONS):
			{
				//Find quaternions
	 ...
	 * @return 		void
	 */
	template <class T> void PlaneParser<T>::getPlaneParameterCovariance(plane_parser_utils::PlaneParameters& plane_parameters, const plane_parser_utils::LeastSquaresMatricies& matricies)
	{
		plane_parameters.covariance_matrix = Eigen::Matrix3f::Zero();
		...
		{
			case (pointcloud_utils::plane_parser_utils::CovarianceType::GOODNESS_OF_FIT_ERROR):
			{
			...
			case (pointcloud_utils::plane_parser_utils::CovarianceType::GOODNESS_OF_FIT_DISTANCE):
			{
			...
			case (pointcloud_utils::plane_parser_utils::CovarianceType::ANALYTICAL):
			{
			...
			case (pointcloud_utils::plane_parser_utils::CovarianceType::MONTE_CARLO):
			{
				std::cout << "Warning: MONTE_Carlo covariance has not yet been implemented.\n";
				break;
			}
			case (pointcloud_utils::plane_parser_utils::CovarianceType::LEAST_SQUARES):
			{
...
// 				//matricies.points_matrix.transpose() * matricies.points_matrix; 
	...
	namespace plane_parser_utils
	{
		PlaneFitType convertPlaneFitType(const std::string& string_in)
		{
			...
			{
				  return pointcloud_utils::plane_parser_utils::PlaneFitType::SIMPLE;
			} else if (string_in == "SVD" || string_in == "svd")
			{
				return pointcloud_utils::plane_parser_utils::PlaneFitType::SVD;
			} else
			{	
				std::cout << "Warning: Unknown plane fit type: " << string_in << "\n";
				return pointcloud_utils::plane_parser_utils::PlaneFitType::UNKNOWN;
			}
		}
		...
		CovarianceType convertCovarianceType(const std::string& string_in)
		{
			...
			{
				return pointcloud_utils::plane_parser_utils::CovarianceType::GOODNESS_OF_FIT_ERROR;
			} else if (string_in == "goodness_of_fit_distance" || string_in == "GOODNESS_OF_FIT_DISTANCE" || string_in == "Goodness_of_Fit_Distance")
			{
				return pointcloud_utils::plane_parser_utils::CovarianceType::GOODNESS_OF_FIT_DISTANCE;
			} else if (string_in == "analytical" || string_in == "ANALYTICAL" || string_in == "Analytical")
			{
				return pointcloud_utils::plane_parser_utils::CovarianceType::ANALYTICAL;
			} else if (string_in == "monte_carlo" || string_in == "MONTE_CARLO" || string_in == "Monte_Carlo")
			{
				return pointcloud_utils::plane_parser_utils::CovarianceType::MONTE_CARLO;
			} else if (string_in == "least_squares" || string_in == "LEAST_SQUARES" || string_in == "Least_Squares")
			{
				return pointcloud_utils::plane_parser_utils::CovarianceType::LEAST_SQUARES;
			} else
			{
				std::cout << "Warning: Unknown covariance type: " << string_in << "\n";
				return pointcloud_utils::plane_parser_utils::CovarianceType::UNKNOWN;
			}
		}
		...
		AngleSolutionType convertAngleSolutionType(const std::string& string_in)
		{
			...
			{
				return pointcloud_utils::plane_parser_utils::AngleSolutionType::ATTITUDE_ANGLES;
			} else if (string_in == "euler_angles" || string_in == "EULER_ANGLES" || string_in == "Euler_Angles")
			{
				return pointcloud_utils::plane_parser_utils::AngleSolutionType::EULER_ANGLES;
			} else if (string_in == "simple_angles" || string_in == "SIMPLE_ANGLES" || string_in == "Simple_Angles")
			{
				return pointcloud_utils::plane_parser_utils::AngleSolutionType::SIMPLE_ANGLES;
			} else if (string_in == "quaternions" || string_in == "QUATERNIONS" || string_in == "Quaternions")
			{
				return pointcloud_utils::plane_parser_utils::AngleSolutionType::QUATERNIONS;
			} else
			{
				std::cout << "Warning: Unknown angle solution type: " << string_in << "\n";
				return pointcloud_utils::plane_parser_utils::AngleSolutionType::UNKNOWN;
			}
		}
