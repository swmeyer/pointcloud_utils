...
ros::Publisher marker_pub; // publisher for visualizatio
...
;			map_width = std::ceil((y_max - y_min) / resolution);
	...
	std::cout << "Skip count: " << skip_count << "\n Pass count: " << pass_count << "\n";
}
...
int main(int argc, char* argv[])
{
    ...
    {
    	ros::Rate(10).sleep();
    	std::cout << "waiting for transform: " << base_frame << " to " << lidar_frame << "\n";
    	ros::spinOnce();
    }
